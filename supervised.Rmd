...
#events <- read.csv(("data/raw/events.csv"), header = FALSE, col.names = features$Name, colClasses = features$Type, na.strings = "-")
...
```{r events_stats4}
#Se intentan eliminar los valores NA pero no va :(
...
df_events[is.na(df_events$dsport)] <- -1
#df_events[is.na(df_events$service)] <- "unknown"
df_events[is.na(df_events$sport)] <- -1
df_events[is.na(df_events$attack_cat)] <- "unknown"
...
```{r data_enrich, echo=FALSE}
#Elimnamos columnas type repetidos
